...
Partial Public Class AI 'i shall thy the Alfie Alphafish (bit optimistic, I know).
    ...
    Private PrimaryPHMValues(1) As Integer
    ...
    Private Sub MakeMove(ByRef Board(,) As Char, ByVal Move As UInt16, ByRef MeCanCastle As CanCastle, ByRef EnemyCanCastle As CanCastle, ByRef KPos As Int16, ByRef MaterialCount() As Integer, ByRef PHMValues() As Integer, ByRef EnPassant As Int16, ByRef ZobristValue As UInt64, ByRef WhitePawnMask As UInt64, ByRef BlackPawnMask As UInt64, ByRef HalfMoveSize As UInt16)
        ...
        If Char.IsUpper(TempPiece) Then
            ...
            'Modifies the PHM values.
            If SearchSettings.UsePieceHeatMaps Then
                If True Then
                    'Modifies Black's PHM.
                    If Move > 32768 Then
                        PHMValues(1) -= PieceHeatMap((Asc(Board(NewCoorX, NewCoorY)) + 1) Mod 11, 7 - NewCoorY, NewCoorX, 16)
                    ElseIf (Move And 28672) = 12288 Then
                        PHMValues(1) -= PieceHeatMap(Asc("P") Mod 11, 4, NewCoorX, 16) 'y=3 for enpassant.
                    End If
                End If
                If True Then
                    'Modifies White's PHM.
                    PHMValues(0) -= PieceHeatMap(Asc(Board(OldCoorX, OldCoorY)) Mod 11, OldCoorY, OldCoorX, 16)
        ...
        Else
            ...
            If SearchSettings.UsePieceHeatMaps Then
                If True Then
                    'Modifies White's PHM.
                    If Move > 32768 Then
                        PHMValues(0) -= PieceHeatMap(Asc(Board(NewCoorX, NewCoorY)) Mod 11, NewCoorY, NewCoorX, 16)
                    ElseIf (Move And 28672) = 12288 Then
                        PHMValues(0) -= PieceHeatMap(Asc("P") Mod 11, 4, NewCoorX, 16) 'y=4 for enpassant.
                    End If
                End If
                If True Then
                    'Modifies Black's PHM.
                    PHMValues(1) -= PieceHeatMap((Asc(Board(OldCoorX, OldCoorY)) + 1) Mod 11, 7 - OldCoorY, OldCoorX, 16)
                    If (Move And 28672) = 4096 Then 'Queen Promotion.
    ...
    Private Function Evaluate(ByRef Board(,) As Char, ByVal MaterialCount() As Integer, ByVal PHMValues() As Integer, ByVal WKPos As Int16, ByVal BKPos As Int16, ByVal WhitePawnMask As UInt64, ByVal BlackPawnMask As UInt64) As Int16
        ...
        If SearchSettings.UsePieceHeatMaps Then
            'Dim ProperPHM() As Integer = GetPHMEval(Board, MaterialCount)
            'If ProperPHM(0) <> PHMValues(0) OrElse ProperPHM(1) <> PHMValues(1) Then
