...
Partial Public Class AI 'i shall thy the Alfie Alphafish (bit optimistic, I know).
    ...
    Public Function Search(ByVal Depth As Integer, Optional ByVal PreviousBestScore As Int16 = -InfScore) As Move
        ...
        If HasBeenInstantiated AndAlso Depth > 0 AndAlso BasePieceMoves IsNot Nothing Then
            ...
            Dim AspirationFails As Integer
            ...
            While True
                ...
                For n = StartValue To EndValue Step StepValue 'for each move...
                    ...
                    MakeMove(TempBoard, BasePieceMoves(n), TempMeCanCastle, TempEnemyCanCastle, TempMeKPos, TempMaterialCount, TempEnPassant, TempZobristValue, TempWhitePawnMask, TempBlackPawnMask, TempHalfMoveSize)
                    DepthFromRoot = 1
                    ...
                    Else
                        ...
                        If (Not SearchSettings.UsePVS) OrElse n = StartValue Then
                            'We are part of the PV search - search with a full window.
                            CurrentScore = -NegaMax(TempBoard, Depth - 1, 0, Not PlayerTurn, TempEnemyCanCastle, TempMeCanCastle, PrimaryEnemyKPos, TempMeKPos, TempEnPassant, TempMaterialCount, TempZobristValue, TempWhitePawnMask, TempBlackPawnMask, TempHalfMoveSize, -Beta, -Alpha, True)
                        Else
                            'The move is not a principle move - search with a null window. Note that we also do not apply Late Move Reductions in the base position.
                            CurrentScore = -NegaMax(TempBoard, Depth - 1, 0, Not PlayerTurn, TempEnemyCanCastle, TempMeCanCastle, PrimaryEnemyKPos, TempMeKPos, TempEnPassant, TempMaterialCount, TempZobristValue, TempWhitePawnMask, TempBlackPawnMask, TempHalfMoveSize, -Alpha - 1S, -Alpha, True)
                            If CurrentScore > Alpha AndAlso CurrentScore < Beta Then
                ...
                If BestMove.Score <= AspirationWindow(0) Then
                    ...
                    AspirationWindow(0) = If(AspirationWindowCode = "Low", -InfScore, AspirationWindow(0) - SearchSettings.AspirationWindowWidth * 3)
                    AspirationWindowCode = "Low"
                ElseIf BestMove.Score >= AspirationWindow(1) Then
                    'We failed high - widen the upper bound.
                    AspirationWindow(1) = If(AspirationWindowCode = "High", InfScore, AspirationWindow(1) + SearchSettings.AspirationWindowWidth * 3)
                ...
                AspirationFails += 1
                If AspirationFails = 3 Then AspirationWindow = {-InfScore, InfScore} 'We have failed too many times - reset the Aspiration Windows.
    ...
    Private Function GenerateBestMoveLine(ByVal BestMove As UInt16, ByVal IsCheckmate As Boolean) As String
        ...
        For i As Byte = 1 To MaxIterations
            ...
            If isWhite Then
                MakeMove(TempBoard, BestMove, TempWCanCastle, TempBCanCastle, TempWKPos, TempMaterialCount, TempEnPassant, TempZobristValue, 0UL, 0UL, 0US)
            Else
                MakeMove(TempBoard, BestMove, TempBCanCastle, TempWCanCastle, TempBKPos, TempMaterialCount, TempEnPassant, TempZobristValue, 0UL, 0UL, 0US)
            End If
    ...
    Public Function ReturnFENAfterMove(ByVal TempMove As Move) As String
        If HasBeenInstantiated Then
            ...
            TempEnemyCanCastle.CopyFrom(PrimaryEnemyCanCastle)
            Try
                MakeMove(TempBoard, TempMove.BitMove, TempMeCanCastle, TempEnemyCanCastle, 0S, {10000, 10000}, TempEnPassant, 0UL, 0UL, 0UL, 0US)
        ...
        If PieceMoves IsNot Nothing Then
            ...
            For n = 0 To PieceMoves.Length - 1
                ...
                If TempMeInCheck = 0 Then 'Therefore, is a legal move.
                    ...
                    MakeMove(TempBoard, PieceMoves(n), TempMeCanCastle, TempEnemyCanCastle, TempMeKPos, {10000, 10000}, TempEnPassant, TempZobristValue, 0UL, 0UL, 0S)
    ...
    Public Sub SortMovesThorough(ByRef Moves() As UInt16, ByRef Board(,) As Char, ByVal isWhite As Boolean, ByVal MeKPos As Int16, ByVal EnemyKPos As Int16, ByVal MeCanCastle As CanCastle, ByVal EnemyCanCastle As CanCastle, ByVal EnPassant As Int16, ByVal ZobristValue As UInt64, ByVal WhitePawnMask As UInt64, ByVal BlackPawnMask As UInt64)
        ...
        For n = 0 To Moves.Length - 1
            ...
            MakeMove(TempBoard, Moves(n), TempMeCanCastle, TempEnemyCanCastle, TempMeKPos, {10000, 10000}, TempEnPassant, TempZobristValue, TempWhitePawnMask, TempBlackPawnMask, 0US)
    ...
    Private Function NegaMax(ByRef Board(,) As Char, ByVal depth As Integer, ByVal NumDepthExt As Integer, ByVal isWhite As Boolean, ByVal MeCanCastle As CanCastle, ByVal EnemyCanCastle As CanCastle, ByVal MeKPos As Int16, ByVal EnemyKPos As Int16, ByVal EnPassant As Int16, ByVal MaterialCount() As Integer, ByVal ZobristValue As UInt64, ByVal WhitePawnMask As UInt64, ByVal BlackPawnMask As UInt64, ByVal HalfMoveSize As UInt16, ByVal Alpha As Int16, ByVal Beta As Int16, ByVal CanTakeNullMove As Boolean) As Int16
        ...
        Else
            ...
            If depth >= NMPRValue AndAlso CanTakeNullMove AndAlso PlayerInCheck < 128 AndAlso Not NoPieceInPos Then
                ...
                If Evaluate(Board, isWhite, MaterialCount, MeKPos, EnemyKPos, WhitePawnMask, BlackPawnMask) < Beta + GlobalConstants.PieceWeight.Pawn Then
                    ActNullMove(EnPassant, ZobristValue)
                    DepthFromRoot += 1
                    'Turn CanTakeNullMove off for the next move, to prevent infinite null moves.
                    BestMove = -NegaMax(Board, depth - NMPRValue, NumDepthExt, Not isWhite, EnemyCanCastle, MeCanCastle, EnemyKPos, MeKPos, 0S, MaterialCount, ZobristValue, WhitePawnMask, BlackPawnMask, HalfMoveSize, -Beta, -Beta + 1S, False)
                    DepthFromRoot -= 1
        ...
        If PieceMoves IsNot Nothing Then 'If any move exists...
            ...
            For n = 0 To PieceMoves.Length - 1 'for each move...
                ...
                If TempPlayerInCheck = 0 Then 'Therefore, is a legal move.
                    ...
                    'Makes move on temp board, then calls NegaMax for this new position.
                    MakeMove(TempBoard, PieceMoves(n), TempMeCanCastle, TempEnemyCanCastle, TempMeKPos, TempMaterialCount, TempEnPassant, TempZobristValue, TempWhitePawnMask, TempBlackPawnMask, TempHalfMoveSize)
                    DepthFromRoot += 1
                    TotalPositionsSearched += 1UL
                    ...
                    Else 'No leaf node or drawn position (or are using Quiescence) - put position through NegaMax recursively.
                        ...
                        If n = 0 Then
                            ...
                            CurrentMove = -NegaMax(TempBoard, depth + DepthExt - 1, NumDepthExt + DepthExt, Not isWhite, TempEnemyCanCastle, TempMeCanCastle, EnemyKPos, TempMeKPos, TempEnPassant, TempMaterialCount, TempZobristValue, TempWhitePawnMask, TempBlackPawnMask, TempHalfMoveSize, -Beta, -Alpha, True)
                        Else
                            ...
                            If Not SearchSettings.StableSearch AndAlso depth >= 3 AndAlso DepthExt = 0 AndAlso NoLegalMoves + If(TempTTEntry.BestMove = 0, 0, 1) >= SearchSettings.MoveReductionThreshold Then
                                ...
                                CurrentMove = -NegaMax(TempBoard, depth - 2, NumDepthExt, Not isWhite, TempEnemyCanCastle, TempMeCanCastle, EnemyKPos, TempMeKPos, TempEnPassant, TempMaterialCount, TempZobristValue, TempWhitePawnMask, TempBlackPawnMask, TempHalfMoveSize, -Alpha - 1S, -Alpha, True)
                            ...
                            If NeedFullSearch Then
                                ...
                                CurrentMove = -NegaMax(TempBoard, depth + DepthExt - 1, NumDepthExt + DepthExt, Not isWhite, TempEnemyCanCastle, TempMeCanCastle, EnemyKPos, TempMeKPos, TempEnPassant, TempMaterialCount, TempZobristValue, TempWhitePawnMask, TempBlackPawnMask, TempHalfMoveSize, If(SearchSettings.UsePVS, -Alpha - 1S, -Beta), -Alpha, True)
