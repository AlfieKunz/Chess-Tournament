...
Partial Public Class AI 'i shall thy the Alfie Alphafish (bit optimistic, I know).
    ...
    Public Function Search(ByVal Depth As Integer, Optional ByVal PreviousBestScore As Int16 = -InfScore) As Move
        ...
        If HasBeenInstantiated AndAlso Depth > 0 AndAlso BasePieceMoves IsNot Nothing Then
            ...
            Dim AspirationFails As Integer = 1
            ...
            While True
                ...
                If BestMove.Score <= AspirationWindow(0) Then
                    'We failed low - widen the lower bound. We do this in an exponential fashion: 1w, 3w, 7w, +-inf.
                    AspirationWindow(0) = If(AspirationFails >= 3, -InfScore, AspirationWindow(0) - DynamicAWWidth * (2 ^ AspirationFails))
                    AspirationWindowCode = "Low"
                ElseIf BestMove.Score >= AspirationWindow(1) Then
                    'We failed high - widen the upper bound.
                    AspirationWindow(1) = If(AspirationFails >= 3, InfScore, AspirationWindow(1) + DynamicAWWidth * (2 ^ AspirationFails))
                ...
                If SearchSettings.OutputToConsole Then
                    ...
                    Console.WriteLine($"Search at Depth {Depth} Broke the Aspiration-Window (Fail-{AspirationWindowCode}). Re-searching with a Larger Window: [{Math.Round(AspirationWindow(0) / 100, 2)},{Math.Round(AspirationWindow(1) / 100, 2)}]...")
    ...
    Private Sub ActNullMove(ByVal EnPassant As Int16, ByRef ZobristValue As UInt64)
        ...
        If EnPassant <> 0S Then ZobristValue = ZobristValue Xor ZobristHashTable(2S, 0S, (EnPassant And 56S) >> 3S, EnPassant And 7S)
        'Changes the player to move on the Zobrist Key.
        ZobristValue = ZobristValue Xor HashConstants(0)
    End Sub
