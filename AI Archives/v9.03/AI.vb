...
Partial Public Class AI 'i shall thy the Alfie Alphafish (bit optimistic, I know).
    ...
    Public Sub Reconfigure(ByVal FEN As String, Optional ByVal ResetTT As Boolean = False)
        ...
        End If
        SortMovesThorough(BasePieceMoves, PrimaryBoard, PlayerTurn, PrimaryMeKPos, PrimaryEnemyKPos, PrimaryMeCanCastle, PrimaryEnPassant, PrimaryZobristValue, PrimaryWhitePawnMask, PrimaryBlackPawnMask)
        ...
        If HasBeenInstantiated AndAlso Depth > 0 AndAlso BasePieceMoves IsNot Nothing Then
            ...
            While True
                ...
                For n = StartValue To EndValue Step StepValue 'for each move...
                    ...
                    MakeMove(TempBoard, BasePieceMoves(n), TempMeCanCastle, TempMeKPos, TempMaterialCount, TempEnPassant, TempZobristValue, TempWhitePawnMask, TempBlackPawnMask, TempHalfMoveSize)
                    DepthFromRoot = 1
                    ...
                    ElseIf Depth = 1 AndAlso Not SearchSettings.UseQuiescence Then
                        ...
                        TotalPositionsSearched += 1UL
                        CurrentScore = Evaluate(TempBoard, PlayerTurn, TempMaterialCount, PrimaryMeKPos, PrimaryEnemyKPos, TempWhitePawnMask, TempBlackPawnMask)
                    Else
                        ...
                        If Not SearchSettings.StableSearch AndAlso Depth >= 4 AndAlso If(SearchSettings.ReturnBestMove, n, BasePieceMoves.Length - n) >= SearchSettings.MoveReductionThreshold + 1 Then
                            CurrentScore = -NegaMax(TempBoard, Depth - 2, 0, Not PlayerTurn, PrimaryEnemyCanCastle, TempMeCanCastle, PrimaryEnemyKPos, TempMeKPos, TempEnPassant, TempMaterialCount, TempZobristValue, TempWhitePawnMask, TempBlackPawnMask, TempHalfMoveSize, -If(SearchSettings.ReturnBestMove, Alpha, Beta) - 1S, -If(SearchSettings.ReturnBestMove, Alpha, Beta), True)
                        ...
                        If NeedFullSearch Then
                            ...
                            CurrentScore = -NegaMax(TempBoard, Depth - 1, 0, Not PlayerTurn, PrimaryEnemyCanCastle, TempMeCanCastle, PrimaryEnemyKPos, TempMeKPos, TempEnPassant, TempMaterialCount, TempZobristValue, TempWhitePawnMask, TempBlackPawnMask, TempHalfMoveSize, -Beta, -Alpha, True)
                        End If
                    End If
    ...
    Private Function GenerateBestMoveLine(ByVal BestMove As UInt16, ByVal IsCheckmate As Boolean) As String
        ...
        For i As Byte = 1 To MaxIterations
            ...
            If isWhite Then
                MakeMove(TempBoard, BestMove, TempWCanCastle, TempWKPos, TempMaterialCount, TempEnPassant, TempZobristValue, 0UL, 0UL, 0US)
            Else
                MakeMove(TempBoard, BestMove, TempBCanCastle, TempBKPos, TempMaterialCount, TempEnPassant, TempZobristValue, 0UL, 0UL, 0US)
            End If
    ...
    Public Function ReturnFENAfterMove(ByVal TempMove As Move) As String
        If HasBeenInstantiated Then
            ...
            Try
                MakeMove(TempBoard, TempMove.BitMove, TempMeCanCastle, 0S, {10000, 10000}, TempEnPassant, 0UL, 0UL, 0UL, 0US)
        ...
        If PieceMoves IsNot Nothing Then
            ...
            For n = 0 To PieceMoves.Length - 1
                ...
                If TempMeInCheck = 0 Then 'Therefore, is a legal move.
                    ...
                    MakeMove(TempBoard, PieceMoves(n), TempMeCanCastle, TempMeKPos, {10000, 10000}, TempEnPassant, TempZobristValue, 0UL, 0UL, 0S)
    ...
    Public Sub SortMovesThorough(ByRef Moves() As UInt16, ByRef Board(,) As Char, ByVal isWhite As Boolean, ByVal MeKPos As Int16, ByVal EnemyKPos As Int16, ByVal CanCastle As CanCastle, ByVal EnPassant As Int16, ByVal ZobristValue As UInt64, ByVal WhitePawnMask As UInt64, ByVal BlackPawnMask As UInt64)
        ...
        If SearchSettings.UsePieceHeatMaps Then OldEval = Evaluate(Board, isWhite, {10000, 10000}, MeKPos, EnemyKPos, WhitePawnMask, BlackPawnMask)
        ...
        For n = 0 To Moves.Length - 1
            ...
            'If the Transposition Table has an entry for the current position, and we are looking at the best move found, give this a very big bonus.
            ...
            MakeMove(TempBoard, Moves(n), TempMeCanCastle, TempMeKPos, {10000, 10000}, TempEnPassant, TempZobristValue, TempWhitePawnMask, TempBlackPawnMask, 0US)

            FixTFTable(TempBoard, Not isWhite, TempTFTable, EnemyKPos, TempInCheck, PrimaryEnemyCanCastle.CanICastle(), TempEnPassant)
            ...
            If SearchSettings.UsePieceHeatMaps Then
                NewEval = Evaluate(TempBoard, Not isWhite, {10000, 10000}, TempMeKPos, EnemyKPos, TempWhitePawnMask, TempBlackPawnMask)
    ...
    Private Sub MakeMove(ByRef Board(,) As Char, ByVal Move As UInt16, ByRef CanCastle As CanCastle, ByRef KPos As Int16, ByRef MaterialCount() As Integer, ByRef EnPassant As Int16, ByRef ZobristValue As UInt64, ByRef WhitePawnMask As UInt64, ByRef BlackPawnMask As UInt64, ByRef HalfMoveSize As UInt16)
        ...
        If Move > 32768 Then
            ...
            If Char.IsUpper(CapturedPiece) Then
                ...
                If CapturedPiece = "P"c Then WhitePawnMask = WhitePawnMask Xor (1UL << (NewCoorY * 8UL + NewCoorX))
            Else
                ...
                If CapturedPiece = "p"c Then BlackPawnMask = BlackPawnMask Xor (1UL << (NewCoorY * 8UL + NewCoorX))
            End If
    ...
    Private Function NegaMax(ByRef Board(,) As Char, ByVal depth As Integer, ByVal NumDepthExt As Integer, ByVal isWhite As Boolean, ByVal MeCanCastle As CanCastle, ByVal EnemyCanCastle As CanCastle, ByVal MeKPos As Int16, ByVal EnemyKPos As Int16, ByVal EnPassant As Int16, ByVal MaterialCount() As Integer, ByVal ZobristValue As UInt64, ByVal WhitePawnMask As UInt64, ByVal BlackPawnMask As UInt64, ByVal HalfMoveSize As UInt16, ByVal Alpha As Int16, ByVal Beta As Int16, ByVal CanTakeNullMove As Boolean) As Int16
        ...
        If Not (depth > 0 OrElse PlayerInCheck >= 128) Then 'Quiescence mode activated.
            ...
            StandPat = Evaluate(Board, isWhite, MaterialCount, MeKPos, EnemyKPos, WhitePawnMask, BlackPawnMask)
        ...
        Else
            ...
            If depth >= SearchSettings.NullMoveRValue + 1 AndAlso PlayerInCheck < 128 AndAlso CanTakeNullMove AndAlso Not NoPieceInPos Then
                ...
                If StandPat >= Beta Then
                    ...
                    BestMove = -NegaMax(Board, depth - SearchSettings.NullMoveRValue - 1, NumDepthExt, Not isWhite, EnemyCanCastle, MeCanCastle, EnemyKPos, MeKPos, 0S, MaterialCount, ZobristValue, WhitePawnMask, BlackPawnMask, HalfMoveSize, -Beta, -Beta + 1S, False)
                    DepthFromRoot -= 1
                    ...
                    If ABORT Then
                        ...
                        If ReplaceTTNode Then
                            If BackupTTEntry.Key > 0 Then TranspositionTable(EntryInTT) = New TTEntry Else TranspositionTable(EntryInTT) = BackupTTEntry
                        End If
                        Return 0
                    End If
        ...
        If PieceMoves IsNot Nothing Then 'If any move exists...
            ...
            For n = 0 To PieceMoves.Length - 1 'for each move...
                ...
                If TempPlayerInCheck = 0 Then 'Therefore, is a legal move.
                    ...
                    'Makes move on temp board, then calls NegaMax for this new position.
                    MakeMove(TempBoard, PieceMoves(n), TempMeCanCastle, TempMeKPos, TempMaterialCount, TempEnPassant, TempZobristValue, TempWhitePawnMask, TempBlackPawnMask, TempHalfMoveSize)
                    DepthFromRoot += 1
                    TotalPositionsSearched += 1UL
                    ...
                    ElseIf Not SearchSettings.UseQuiescence AndAlso depth = 1 Then
                        'We have reached a leaf position - return the evaluation for this position.
                        CurrentMove = Evaluate(TempBoard, isWhite, TempMaterialCount, TempMeKPos, EnemyKPos, TempWhitePawnMask, TempBlackPawnMask) 'Evaluate position for opponent.
                    Else 'No leaf node or drawn position (or are using Quiescence) - put position through NegaMax recursively.
                        ...
                        If Not SearchSettings.StableSearch AndAlso depth >= 3 AndAlso DepthExt = 0 AndAlso NoLegalMoves + If(TempTTEntry.BestMove = 0, 0, 1) >= SearchSettings.MoveReductionThreshold Then
                            ...
                            CurrentMove = -NegaMax(TempBoard, depth - 2, NumDepthExt, Not isWhite, EnemyCanCastle, TempMeCanCastle, EnemyKPos, TempMeKPos, TempEnPassant, TempMaterialCount, TempZobristValue, TempWhitePawnMask, TempBlackPawnMask, TempHalfMoveSize, -Alpha - 1S, -Alpha, True)
                            ...
                            End If
                        End If
                        If NeedFullSearch Then CurrentMove = -NegaMax(TempBoard, depth + DepthExt - 1, NumDepthExt + DepthExt, Not isWhite, EnemyCanCastle, TempMeCanCastle, EnemyKPos, TempMeKPos, TempEnPassant, TempMaterialCount, TempZobristValue, TempWhitePawnMask, TempBlackPawnMask, TempHalfMoveSize, -Beta, -Alpha, True)
                    End If
    ...
    Private Function Evaluate(ByRef Board(,) As Char, ByVal isWhite As Boolean, ByVal MaterialCount() As Integer, ByVal MeKPos As Int16, ByVal EnemyKPos As Int16, ByVal WhitePawnMask As UInt64, ByVal BlackPawnMask As UInt64) As Int16
        'Overload for the Evaluation function, for use in NegaMax.
        If isWhite Then
            Return Evaluate(Board, MaterialCount, MeKPos, EnemyKPos, WhitePawnMask, BlackPawnMask)
        Else
            Return -Evaluate(Board, MaterialCount, EnemyKPos, MeKPos, WhitePawnMask, BlackPawnMask) '- as '-1' is good for black, but bad for white.
        End If
    End Function
    Private Function Evaluate(ByRef Board(,) As Char, ByVal MaterialCount() As Integer, ByVal WKPos As Int16, ByVal BKPos As Int16, ByVal WhitePawnMask As UInt64, ByVal BlackPawnMask As UInt64) As Int16
        ...
        If SearchSettings.UsePieceHeatMaps Then
            ...
            For y = 0 To 7
                For x = 0 To 7
                    If Board(x, y) <> " "c Then
                        If Char.IsUpper(Board(x, y)) Then
                            Score += PieceHeatMap(Asc(Board(x, y)) Mod 11, y, x, ScaledMaterialCountB)
                        Else
                            Score -= PieceHeatMap((Asc(Board(x, y)) + 1) Mod 11, 7 - y, x, ScaledMaterialCountW)
                        End If
                    End If
                Next
            Next
        ...
        If MaterialCount(0) <= 1400 Then
            ...
            Score -= EndgameEvalLookupTable(WKPos, BKPos, ScaledMaterialCountW)
        End If
        If MaterialCount(1) <= 1400 Then
            'Black has little material left.
            Score += EndgameEvalLookupTable(BKPos, WKPos, ScaledMaterialCountB)
        End If
